...
async function scrapFunction(givenDate) {
  ...
  let appendHeader = {
    ...
    "Date From": "Start",
    "Date To": "End",
    "Total(pay now)": "Pay Now",
    "Total(pay later)": "Pay Later",
    Link: "Reserve",
  };
  ...
  async function handleScraping() {
    for (let i = 0; i < 30; i++) {
      ...
      async function getRequestData() {
        ...
        page.on("request", async (request) => {
          if (!request.isNavigationRequest()) {
            ...
            if (
              request
                .url()
                .includes("https://www.avis.com/webapi/reservation/vehicles")
            ) {
        ...
        await delay(3000);
        const endDateField = await page.waitForSelector("#to", {
          timeout: 300000,
        });
        ...
        await delay(3000);
        const endDateXPath =
          '//*[@id="ui-datepicker-div"]/div[2]/table/tbody/tr[5]/td[3]/a';
        ...
        const findButton = await page.waitForSelector("#res-home-select-car", {
          timeout: 500000,
        });
        ...
        async function processFilesSequentially(filePaths) {
          try {
            ...
            for (const row of data) {
              ...
              const location =
                locationElements[elementNum - 5] +
                locationElements[elementNum - 4] +
                locationElements[elementNum - 3] +
                locationElements[elementNum - 2] +
                locationElements[elementNum - 1];
              ...
              await fetch("https://www.avis.com/webapi/reservation/vehicles", {
                ...
                body:
                  '{"rqHeader":{"brand":"","locale":"en_US"},"nonUSShop":false,"pickInfo":"' +
                  searchKey +
                  '","pickCountry":"US","pickDate":"' +
                  start_date +
                  '","pickTime":"12:00 PM","dropInfo":"' +
                  searchKey +
                  '","dropDate":"' +
                  end_date +
                  '","dropTime":"12:00 PM","couponNumber":"","couponInstances":"","couponRateCode":"","discountNumber":"B257500","rateType":"LEISURE","residency":"US","age":25,"wizardNumber":"","lastName":"","userSelectedCurrency":"","selDiscountNum":"","promotionalCoupon":"","preferredCarClass":"","membershipId":"","noMembershipAvailable":false,"corporateBookingType":"","enableStrikethrough":"true","picLocTruckIndicator":false,"amazonGCPayLaterPercentageVal":"","amazonGCPayNowPercentageVal":"","corporateEmailID":""}',
                method: "POST",
              })
                .then((response) => {
                  if (response.ok) {
                  ...
                  for (let num = 0; num < infos.length; num++) {
                    ...
                    if (info.payLaterRate) {
                      ...
                      result = {
                        ...
                        PayLaterTotal: payLaterTotalAmount,
                        State: state,
                        City: city,
                      ...
                      const appendData = {
                        ...
                        Address: location,
                        Class: carName,
                        "Vehicle type": carDesc,
                        "Date From": startDate,
                        "Date To": endDate,
                        "Total(pay now)": payNowTotalAmount,
                        "Total(pay later)": payLaterTotalAmount,
                        Link: link,
                      };
